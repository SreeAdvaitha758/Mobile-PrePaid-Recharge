...
public class AdminService {
   ...
   public Admin login(String username, String password) {
    ...
    public List<Subscriber> getExpiringPlans() {
        ...
        // Add current plan info if needed
    ...
    public void checkAndNotifyInactiveSubscribers() {
        Calendar cal = Calendar.getInstance();
        cal.add(Calendar.YEAR, -1); // 1 year ago
        ...
        for (Subscriber subscriber : inactiveSubscribers) {
            ...
            // Schedule deactivation check (simplified here; use a scheduler like Spring @Scheduled in production)
            new Thread(() -> {
                try {
                    Thread.sleep(30 * 24 * 60 * 60 * 1000L); // 30 days
