...
public class SubscriberController {
    ...
    public ResponseEntity<VerifyNumberResponse> verifyNumber(@RequestBody OTPRequest request) {
        ...
        if (subscriber != null && "ACTIVE".equals(subscriber.getStatus())) {
            return ResponseEntity.ok(new VerifyNumberResponse(
                    "success",
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new VerifyNumberResponse(
                        "error",
                        "Number not registered or inactive",
                        null
                ));
    }
    ...
    public ResponseEntity<Map<String, String>> verifyOTP(@RequestBody OTPRequest request) {
        try {
            ...
            if (isValid) {
                Subscriber subscriber = subscriberRepository.findByMobileNumber(request.getMobileNumber());
                String token = jwtUtil.generateToken(subscriber.getMobileNumber(), "SUBSCRIBER");
                Map<String, String> response = new HashMap<>();
